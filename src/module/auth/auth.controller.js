...
export const signinCheck = async (req, res) => {
  try {
    ...
    if (!clientId) res.status(401).json({ message: 'Unauthorized - Authentication required' });
...
export const signIn = async (req, res) => {
  try {
    ...
    if (!user.password) {
      logger.error(`Sign In failed: Password is missing for user - ${identifier}`);
    ...
    const safeUser = user.toObject ? getUserToUserTokenDto(user.toObject()) : {};
    let response;
    if (clientId === 'ADMIN') {
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const refreshToken = isMobile ? req.body.refreshToken : req.cookies.refreshToken;
    if (!refreshToken) {
    ...
    const accessToken = await AuthService.generateToken(user);
    const userData = user.toObject ? getUserToUserTokenDto(user.toObject()) : {};
...
export const signOut = async (req, res)=> {
  try {
    ...
    const refreshToken = isMobile ? req.body.refreshToken : req.cookies.refreshToken;
    if (!refreshToken) {
      logger.error(`Signout failed: No refresh token provided (Client: ${clientId})`);
    ...
    logger.info(`User signed out successfully (Client: ${clientId}, User Id: ${decoded.id})`);
