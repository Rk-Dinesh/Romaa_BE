import DrawingVsBOQDEService from "./drawingvsboqDE.service.js";
import fs from "fs";
...
export const detailedEstimateCustomHeading = async (req, res) => {
  try {
    ...
    const result =
      await DrawingVsBOQDEService.createDetailedEstimateCustomHeadings(
        tender_id,
        req.body,
      );
    res
...
export const extractHeadingInpairs = async (req, res) => {
  try {
    const tender_id = req.query;
    const result =
      await DrawingVsBOQDEService.extractHeadingsInPairs(tender_id);
    res
      .status(200)
      .json({
        status: true,
  ...
  try {
    ...
    fs.createReadStream(filePath)
      ...
      .on("end", async () => {
        try {
          const result =
            await DrawingVsBOQDEService.bulkInsertCustomHeadingsFromCsv(
              tender_id,
              nametype,
              csvRows,
            );
...
export const bulkInsertCustomHeadingsControllerNew = async (req, res, next) => {
  ...
  try {
    ...
    const result =
      await DrawingVsBOQDEService.bulkInsertCustomHeadingsFromCsvNew(
        tender_id,
        nametype,
        dataRows,
      );
...
export const getCustomHeadingsByTenderAndNameTypeController = async (
  ...
  try {
    ...
    const data =
      await DrawingVsBOQDEService.getCustomHeadingsByTenderAndNameTypeService(
        tender_id,
        nametype,
      );
    return res
...
export const getGeneralAbstractController = async (req, res) => {
  try {
    const { tender_id } = req.query;
    const data =
      await DrawingVsBOQDEService.getGeneralAbstractService(tender_id);
    return res
      .status(200)
      .json({ status: true, message: "Data retrieved successfully", data });
  } catch (error) {
...
export const getBillOfQtyController = async (req, res) => {
  try {
    const { tender_id } = req.query;
    const data = await DrawingVsBOQDEService.getBillOfQtyService(tender_id);
    return res
      .status(200)
      .json({ status: true, message: "Data retrieved successfully", data });
  } catch (error) {
...
export const addPhaseBreakdownToAbstractController = async (req, res) => {
  try {
    ...
    const data =
      await DrawingVsBOQDEService.addPhaseBreakdownToAbstractService(
        tender_id,
        nametype,
        description,
        phase,
...
export const addPhaseBreakdownToDetailedController = async (req, res) => {
  try {
    ...
    const { description, phase, quantity } = req.body;
    const data =
      await DrawingVsBOQDEService.addPhaseBreakdownToDetailedService(
        tender_id,
        nametype,
        description,
        phase,
