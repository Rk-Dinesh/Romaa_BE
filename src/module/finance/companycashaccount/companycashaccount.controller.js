...
export const create = async (req, res) => {
  try {
    ...
    res.status(201).json({ status: true, message: "Company cash account registered successfully", data });
  } catch (error) {
...
export const update = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: true, message: "Company cash account updated successfully", data });
  } catch (error) {
    const code = error.message.includes("not found") ? 404 : 500;
    res.status(code).json({ status: false, message: error.message });
  }
...
export const softDelete = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: true, message: "Company cash account deactivated successfully", data });
  } catch (error) {
