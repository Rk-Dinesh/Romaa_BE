    ...
    static async bulkInsert(csvRows, tender_id, meta = {}) {
        ...
        try {
            ...
            if (processedCSVItems.length === 0) {
                throw new Error("Zero items extracted. Check CSV headers (Code, Quantity, Description).");
            }
            ...
            }).select("bill_id status").session(session);
            if (pendingBill) {
                throw new Error(
...
static async updateBillByCSV(csvRows, bill_id, meta = {}) {
    ...
    try {
        ...
        if (bill.status !== "Draft") throw new Error(`Bill ${bill_id} is "${bill.status}" — only Draft bills can be edited`);
        ...
        for (const row of csvRows) {
            ...
            if (level === 1 && code !== "") {
                currentParentCode = code;
                if (!itemsMap.has(code)) {
                    itemsMap.set(code, { item_code: code, item_name: desc, unit: getValue(row, "Unit"), quantity: qty, mb_book_ref: mb || "" });
                } else {
                    itemsMap.get(code).quantity += qty;
                }
            } else if (currentParentCode) {
        ...
        const processedCSVItems = Array.from(itemsMap.values());
        if (processedCSVItems.length === 0) throw new Error("Zero items extracted. Check CSV headers (Code, Quantity, Description).");

        // Fetch bid for rates
        const bidDoc = await BidModel.findOne({ tender_id }).session(session);
        if (!bidDoc) throw new Error(`Agreement (Bid) not found for Tender: ${tender_id}`);

        // Fetch previous bill (most recent) for cumulative quantities
        const prevBill = await BillingModel.findOne({ tender_id, _id: { $ne: bill._id } })
            .sort({ createdAt: -1 }).session(session);
        const prevMap = new Map();
        if (prevBill) {
            prevBill.items.forEach(i => prevMap.set(i.item_code.toString().trim().toUpperCase(), i.upto_date_qty || 0));
        }
        ...
        const finalItems = bidDoc.items.map(bidItem => {
            ...
            const csvMatch = processedCSVItems.find(i => i.item_code?.toString().trim().toUpperCase() === bidCode);
            return {
...
static async createBill(data) {
    ...
    // 1. Fetch Tender Data
    const tenderDoc = await TenderModel.findOne({ tender_id }).session(_session);
    if (!tenderDoc) throw new Error(`Tender details not found for ID: ${tender_id}`);
    ...
    const finalItems = bidDoc.items.map(bidItem => {
        ...
        const csvMatch = items.find(i => 
            i.item_code?.toString().trim().toUpperCase() === bidCode
        );
