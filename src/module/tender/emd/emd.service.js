...
class EmdService {
  ...
  static async addProposalToTender(
    ...
    if (!tender_id)
      throw new Error("Tender ID is required to process this EMD request.");
    if (!proposal || !proposal.company_name)
      throw new Error("Company name is required to submit an EMD proposal.");

    const tender = await TenderModel.findOne({ tender_id });
    if (!tender)
      throw new Error("Tender record not found for the specified Tender ID.");

    const emdApprovedFound = await EmdModel.findOne({ tender_id });
    if (emdApprovedFound?.proposals?.some((p) => p.status === "APPROVED")) {
      throw new Error("EMD record already approved for this tender.");
    }
    ...
    if (!emdRecord) {
      ...
      if (!emd_id)
        throw new Error(
          "Failed to generate EMD reference number. Please try again.",
        );
  ...
  static async updateEmdRecord(tender_id, updateData) {
    ...
    if (updateData.proposals) {
      updateData.proposals = updateData.proposals.map((p) => ({
        ...
        emd_amount:
          p.proposed_amount && tender.emd?.emd_percentage
  ...
  static async updateProposalInTender(tender_id, proposal_id, updateData) {
    ...
    const index = emdRecord.proposals.findIndex(
      (p) => p.proposal_id === proposal_id,
    );
    if (index === -1)
      throw new Error("EMD proposal not found for the specified Proposal ID.");
    ...
    if (updateData.proposed_amount && tender.emd?.emd_percentage) {
      ...
      updateData.emd_amount =
        (updateData.proposed_amount * tender.emd.emd_percentage) / 100;
    }
  ...
  static async updateProposalWithApprovalRule(
    ...
    if (!emdDoc) {
      throw new Error(
        `No Earnest Money Deposit record found for Tender ID ${targetTenderId}.`,
      );
    }

    const currentProposal = emdDoc.proposals.find(
      (p) => p.proposal_id === targetProposalId,
    );
    if (!currentProposal) {
      throw new Error(
        "The specified Proposal ID does not exist in this tender's EMD record.",
      );
    }

    const emdApprovedFound = emdDoc.proposals.find(
      (p) => p.status === "APPROVED",
    );
    if (emdApprovedFound) {
      throw new Error("EMD already approved for this tender");
    }
    ...
    if (status === "APPROVED") {
      ...
      const updateResult = await EmdModel.updateOne(
        { tender_id: targetTenderId },
        {
          $set: {
            ...
            "proposals.$[others].rejection_reason":
              "L1 bidder selected; remaining proposals have been automatically rejected as per evaluation criteria.",
          },
        },
        {
          arrayFilters: [
      ...
      try {
          ...
          { tender_id: targetTenderId },
          {
            $set: {
              "emd.approved_emd_details": {
                ...
                emd_deposit_pendingAmount: currentProposal.emd_amount,
                ...
                security_deposit_pendingAmount: sdAmount,
        ...
        throw new Error(
          "Failed to sync approved EMD details to the Tender record. Please contact the system administrator.",
        );
      }
    } else {
      // 4. REJECTION LOGIC
      const simpleUpdate = await EmdModel.updateOne(
        {
          tender_id: targetTenderId,
          "proposals.proposal_id": targetProposalId,
        },
        {
          $set: {
            ...
            "proposals.$.rejection_reason":
              status === "REJECTED"
                ? "Proposal rejected manually by the authorized approver."
                : "",
          },
        },
      );
