...
export const startAbsenteeismCron = () => {
  ...
  cron.schedule("59 23 * * *", async () => {
    ...
    try {
      ...
      const employees = await EmployeeModel.find({ status: "Active" }).select("_id shiftType");
      ...
      for (const emp of employees) {
        ...
        if (!attendance) {
          ...
          if (approvedLeave) {
            ...
            if (approvedLeave.requestType.includes("Half")) {
             ...
             finalStatus = "On Leave"; // Or "Holiday"
        ...
        if (!attendance.lastOut) {
            ...
            if (openSession) {
                ...
                openSession.durationMins = Math.round((autoOutTime - openSession.startTime) / 60000);
